...
import { getClientById, TClientById } from "@/app/_actions/client"; // Ajuste o caminho conforme necessário
...
import InvestmentsInactivePage from '../../_components/investmentInactiveList';
import RouterBackButton from '@/components/router-back-button';

export default function SingleClientPage({ params }: { params: { clientId: string } }) {
    const [client, setClient] = useState<TClientById | null>(null); // Ajuste o tipo conforme o formato dos dados
        ...
        <main className="bg-gray-50 min-h-screen p-6">
            <div className="flex justify-between items-center mb-4">
                ...
                <HelperDialog title='Ajuda'>
                    <div>
                        Esta página permite que você visualize as informações detalhadas do perfil do cliente, como dados pessoais, familiares e investimentos.
                    </div>
                </HelperDialog>
            </div>
            ...
            <Card className="p-6 max-w-full mx-auto bg-white shadow-md rounded-lg">
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                    <TabsList className="border-b border-gray-300 mb-4">
                        ...
                        <TabsTrigger value="InvestimentosInativos" className="py-2 px-4 text-gray-400 hover:text-blue-500 focus:outline-none focus:ring-0">
                    ...
                    {activeTab === "Cliente" && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div>
                                <Label htmlFor="partnerId" className="text-gray-700">Nome do Parceiro</Label>
                                <Input id="partnerId" value={client.user.nome} className="bg-gray-100 text-gray-800" readOnly />
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === "InvestimentosInativos" && (
                        <div>
                            <InvestmentsInactivePage clientId={params.clientId} />
                        </div>
                    )}
                </Tabs>
            </Card>
